...
impl AgentRender for AntVis {
    fn sprite(&self, _agent: &Box<dyn Agent>, _state: &Box<&dyn State>) -> SpriteType {
        let agent = _agent.downcast_ref::<Ant>().unwrap();
        match agent.ant_type {
            AntType::RED => SpriteType::Emoji(String::from("red_ant")),
            AntType::BLACK => SpriteType::Emoji(String::from("black_ant")),
        }
    }
